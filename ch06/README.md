...
Weaight decayは、全ての重みに対して、損失関数にL2ノルム（1/2(λW**2)）を加算する。  
重みも勾配を求める計算では、誤差逆伝搬法による結果に、正則化項の微分λWを加算する。

### Dropout
過学習を抑制する方法の一つ。  
ニューロンをランダムに消去しながら学習する手法。  
訓練時に隠れそうのニューロンをランダムに選び出し、その選びだしたニューロンを消去する。  
ニューラルネットワークのモデルが複雑になるとWeight decayだけでは対応が困難になってくるためこの手法が用いられる。  
